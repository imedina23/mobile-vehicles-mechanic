    ...
    public class UsersController : Controller
    {
        ...
        public async Task<IActionResult> Create(UserViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Response response = _mailHelper.SendMail(model.Email, "Vehicles - Account confirmation", $"<h1>Vehicles - Account confirmation</h1>" +
                    $"To enable the user, " +
        ...
        public async Task<IActionResult> AddVehicle(VehicleViewModel vehicleViewModel)
        {
            ...
            catch (DbUpdateException dbUpdateException)
            {
                ...
                {
                    ModelState.AddModelError(string.Empty, "There is already a vehicle with that plate.");
                }
                else
                {
